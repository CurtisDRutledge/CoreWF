...
{
    public IList<NamespaceDeclaration> ParentNamespaces { get; set; }

    public XamlNodeList Nodes { get; init; }

    // Back-compat switch: we don't want to copy parent settings on Activity/DynamicActivity
    public bool IgnoreParentSettings { get; set; }
    ...
    public abstract Func<object> GetFunc();

    public static FuncFactory CreateFactory(XamlReader xamlReader, IServiceProvider context)
    {
...
public class FuncFactory<T> : FuncFactory where T : class
{
    ...
    public override Func<object> GetFunc()
    {
        return (Func<T>) Evaluate;
    }

    public Func<T> GetTypedFunc()
    {
        return Evaluate;
    }
