...
public sealed class InOutArgument<T> : InOutArgument
{
    ...
    public override ActivityWithResult ExpressionCore
    {
        get => Expression;
        set
        {
    ...
    public override Location CreateDefaultLocation() => CreateLocation<T>();

    public override void Declare(LocationEnvironment targetEnvironment, ActivityInstance activityInstance)
        => targetEnvironment.DeclareTemporaryLocation<Location<T>>(RuntimeArgument, activityInstance, false);

    public override bool TryPopulateValue(LocationEnvironment targetEnvironment, ActivityInstance targetActivityInstance, ActivityExecutor executor)
    {
