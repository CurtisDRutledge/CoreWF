...
public abstract class LocationReference
{
    ...
    public virtual Location GetLocationForRead(ActivityContext context) => GetLocation(context);

    // The contract of this method is the caller promises never to read from the location
    public virtual Location GetLocationForWrite(ActivityContext context) => GetLocation(context);
}
