...
public class CodeActivityContext : ActivityContext
{
    ...
    public CodeActivityContext() { }

    // This is only used by base classes which do not take
    // part in pooling.
    public CodeActivityContext(ActivityInstance instance, ActivityExecutor executor)
        : base(instance, executor) { }

    public void Initialize(ActivityInstance instance, ActivityExecutor executor) => Reinitialize(instance, executor);
